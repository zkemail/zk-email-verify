...
contract VerifiedTwitterEmail is ERC721Enumerable, Verifier {
    ...
    function convert7PackedBytesToBytes(uint256[] memory packedBytes)
        public
        view
        returns (string memory extractedString)
    {
        ...
        console.log(state);
        console.logBytes(nonzeroBytesArray);
        console.logString(returnValue);
        require(state == 2, "Invalid final state of packed bytes in email");
        require(
            nonzeroBytesArrayIndex <= 15,
            "Twitter username more than 15 chars!"
        );
        return returnValue;
    ...
    // TODO: Remove console.logs and define this as a pure function instead of a view
    function _domainCheck(uint256[] memory headerSignals)
        public
        view
        returns (bool)
    {
        string memory senderBytes = convert7PackedBytesToBytes(headerSignals);
        string[2] memory domainStrings = [
            "verify@twitter.com",
            "info@twitter.com"
        ];
        return
            _stringEq(senderBytes, domainStrings[0]) ||
            _stringEq(senderBytes, domainStrings[1]);
