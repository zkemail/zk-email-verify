...
async function resolveDNSHTTP(name, type) {
    const resp = await fetch('https://dns.google/resolve?' + new URLSearchParams({
        name: name,
        type: type
    }));
...
const getPublicKey = async (type, name, minBitLength, resolver) => {
    ...
    if (rr) {
        ...
        const publicKeyObj = await importRsaKey(publicKeyPem.toString());
        /*const publicKeyObj = crypto.createPublicKey({
            key: publicKeyPem,
            format: 'pem'
        });*/

        let keyType = publicKeyObj.algorithm.name.split('-')[0].toLowerCase();
