    ...
    stream.on("end", function () {
      ...
      localforage.setItem(rawFilename, extractedData, function (err: Error) {
        if (err) {
          ...
          console.log(`Saved ${rawFilename} file to localForage`);
        }
      });
    });
  });
...
export async function generateProof(input: any, filename: string) {
  ...
  console.log(input);
  localforage.iterate(function (value: any, key: string, iterationNumber: any) {
    // Get the size of the current value in bytes
    localforage
      .getItem(key)
    ...
    console.log(typeof key, typeof value, typeof iterationNumber);
