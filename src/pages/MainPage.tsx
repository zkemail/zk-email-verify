  ...
  return (
		<Container>
			{showBrowserWarning && (
				<TopBanner
					message={
						'ZK Email only works on Chrome or Chromium-based browsers.'
					}
				/>
			)}
			<div className="title">
			...
			>
				<span style={{ color: 'rgba(255, 255, 255, 0.7)' }}>
					...
					and debugging this page, it is likely unstable. Due to
					download limits of incognito mode and non-chrome browsers,
					you must use Chrome to generate proofs right now. Our goal
					for March 2023 is to make this process 10x faster and
					smaller. If you wish to generate a ZK proof of Twitter
					badge, you must do these:
				</span>
				<NumberedStep step={1}>
					Send yourself a{' '}
					<a
				...
				<NumberedStep step={5}>
					Click <b>"Generate Proof"</b>. Since it is completely client
					side and open source, and you are not trusting us with any
					private information.
				</NumberedStep>
				<NumberedStep step={6}>
					Click <b>"Verify"</b> and then{' '}
					<b>"Mint Twitter Badge On-Chain"</b>, and approve to mint
					the NFT badge that proves Twitter ownership! Note that it is
					700K gas right now so only feasible on Goerli, though we
					...
					<Button
						...
						onClick={async () => {
							...
							const { proof, publicSignals } =
								await generateProof(input, filename);
							...
							let kek = publicSignals.map((x: string) =>
								BigInt(x)
							);
							let soln = packedNBytesToString(kek.slice(0, 12));
							let soln2 = packedNBytesToString(
								kek.slice(12, 147)
							);
							let soln3 = packedNBytesToString(
								kek.slice(147, 150)
							);
