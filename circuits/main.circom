...
template RSAGroupSigVerify(n, k, levels) {
    ...
    signal input base_message[k];
    signal input padded_message[k];
    signal input payload; // do nothing
    ...
    component poseidonK = PoseidonK(k);
    for (var i = 0; i < k; i++) {
        poseidonK.inputs[i] <== modulus[i];
    }
    poseidonK.out === leaf;
}
